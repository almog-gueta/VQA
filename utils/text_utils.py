...
def process_digit_article(inText):
    ...
    for word in tempText:
        word = manual_map.setdefault(word, word) # convert from num as str to num as numeric: 'one'->'1'
...
def filter_answers(answers_dset, min_occurence):
    ...
    for ans_entry in answers_dset:
        ...
        gtruth = process_digit_article(process_punctuation(gtruth))
...
def create_ans2label(occurence, name, cache_root):
    ...
    main_utils.make_dir(cache_root)
    ...
    ***answers_dset should be preprocessed***
    ...
    for ans_entry in answers_dset:
        ...
        for answer in answers:
            answer_ = process_digit_article(process_punctuation(answer['answer']))
        ...
        label_scores = {} # we changed label_counts to label_scores- get_score(v) instead of v
        for k, v in answer_count.items():
            if k in ans2label:
                label_scores[ans2label[k]] = get_score(v)
    ...
    print("num of answers that didnt pass the threshold: ", count_empty_a)
    # main_utils.make_dir(cache_root)
    # cache_file = os.path.join(cache_root, name+'_target.pkl')
    # print('path to target file: ', cache_file)
...
def load_v2(data_name, cfg):
    ...
    occurence = filter_answers(train_answers, cfg['dataset']['filter_ans_threshold'])
    ans2label = create_ans2label(occurence, 'train', "./data/cache") # "/home/student/hw2/data/cache"
    if data_name == 'train':
        target = compute_target(train_answers, ans2label, 'train', "./data/cache") # "/home/student/hw2/data/cache"
    else:
        target = compute_target(val_answers, ans2label, 'val', "./data/cache") # "/home/student/hw2/data/cache"

    return target, len(occurence)
